...
export function GoogleButton({ next, label }: { next?: string; label?: string }) {
  ...
  return (
    <Link
      href={href}
      className="border-border bg-card text-foreground hover:bg-muted flex h-10 w-full items-center justify-center gap-2.5 rounded-lg border text-sm font-medium transition-colors"
    >
      <GoogleMark />
